...
class VideoMotion {
    ...
    getBrightness() {
        ...
        let sum = 0;
        function convertLuminance(pixel) {
            return  0.298912 * (pixel & 0xff000000 >>> 24) + 0.586611 * (pixel & 0x00ff0000 >>> 16) +  0.114478 * (pixel & 0x0000ff00 >>> 8)}
        for(let i=0; i < this.curr.length; i++) {
            sum += convertLuminance(this.curr[i])
        }
        return Math.floor(sum / this.curr.length)
    }
    /**
